...
ASMSTracker::ASMSTracker()
{
	nFrames = -1;
}
void ASMSTracker::InputROI(Rect groundtruth)
{
	roi = groundtruth;
}

void ASMSTracker::InputImage(Mat orig)
{
	nFrames += 1;
	image = orig;
}

void ASMSTracker::RunTracking()
{
	// First frame, give the groundtruth to the tracker
	if (nFrames == 0) {
		init(image, roi.x, roi.y, roi.x + roi.width, roi.y + roi.height);
	}
	// Update
	else{
		track(image);
...
bool ASMSTracker::UpdateResult(std::string title)
{
	if (showing)
	{
		if (nFrames == 0)
			rectangle(image, Point(roi.x, roi.y), Point(roi.x + roi.width, roi.y + roi.height), Scalar(0, 255, 255), 1, 8);
		else
		{
			BBox * bb = getBBox();
			rectangle(image, Point(bb->x, bb->y), Point(bb->x + bb->width, bb->y + bb->height), Scalar(0, 255, 255), 1, 8);
		}
...
void ASMSTracker::init(cv::Mat & img, int x1, int y1, int x2, int y2)
{
...
cv::Point ASMSTracker::histMeanShift(double x1, double y1, double x2, double y2)
{
    int maxIter = 15;
...
cv::Point ASMSTracker::histMeanShiftIsotropicScale(double x1, double y1, double x2, double y2, double * scale, int * iter)
{
    int maxIter = 15;
...
cv::Point ASMSTracker::histMeanShiftAnisotropicScale(double x1, double y1, double x2, double y2, double * width, double * height)
{
    int maxIter = 15;
...
BBox * ASMSTracker::track(cv::Mat & img, double x1, double y1, double x2, double y2)
{
    double width = x2-x1;
    double height = y2-y1;
...
void ASMSTracker::preprocessImage(cv::Mat &img)
{
    cv::Mat ra[3] = {im1, im2, im3};
    cv::split(img, ra);
}

void ASMSTracker::extractBackgroundHistogram(int x1, int y1, int x2, int y2, Histogram & hist)
{
    int offsetX = (x2-x1)/2;
    int offsetY = (y2-y1)/2;
...
void ASMSTracker::extractForegroundHistogram(int x1, int y1, int x2, int y2, Histogram & hist)
{
    hist.clear();
