...
void LucasKanadeTracker::lowpass_filter(unsigned char*&src, const int H, const int W, unsigned char*&smoothed)
{
	//tackle with border  
	for (int i = 0; i < H; i++)
	{
	...
	for (int j = 1; j < W - 1; j++)
	{
		...
		re += src[(i - 1)*W + j + 1] * 0.0625;
		re += src[(i + 1)*W + j + 1] * 0.0625;
		smoothed[i*W + j] = unsigned char(re);
	}
	delete[]src;
	src = smoothed;
}
...
void LucasKanadeTracker::get_info(const int nh, const int nw)
{
	...
	if (isusepyramid)
		...
		max_pyramid_layer = 1;
	pre_pyr = new unsigned char*[max_pyramid_layer];
	next_pyr = new unsigned char*[max_pyramid_layer];
...
unsigned char*&LucasKanadeTracker::get_pyramid(int th)
{
	return pre_pyr[th];
}
LucasKanadeTracker::Point LucasKanadeTracker::get_result()
{
	Point pp;
	pp.x = target[0].x;
	pp.y = target[0].y;
...
void LucasKanadeTracker::get_pre_frame(unsigned char*&gray)//use only at the beginning  
{
	pre_pyr[0] = gray;
...
void  LucasKanadeTracker::get_next_frame(unsigned char*&gray)
{
	next_pyr[0] = gray;
...
void LucasKanadeTracker::pyramid_down(unsigned char*&src_gray_data,
	const int src_h, const int src_w, unsigned char*& dst, int&dst_h, int&dst_w)
{
	dst_h = src_h / 2;
	dst_w = src_w / 2;
	int ii = height[1];
	...
	dst = new unsigned char[dst_h*dst_w];
...
double LucasKanadeTracker::get_subpixel(unsigned char*&src, int h, int w, const Point& point)
{
...
void LucasKanadeTracker::build_pyramid(unsigned char**&pyramid)
{
...
void LucasKanadeTracker::lucaskanade(unsigned char**&frame_pre, unsigned char**&frame_cur,
	Point*& start, Point*& finish, unsigned int point_nums, char*state)
{
		...
		for (int j = max_pyramid_layer - 1; j >= 0; j--)
		{
			...
			for (double yy = Yleft; yy < Yright + 0.01; yy += 1.0)
			{
				...
				double derivativeX = get_subpixel(frame_pre[j],
					height[j], width[j], Point(xx + 1.0, yy)) -
					get_subpixel(frame_pre[j], height[j],
					width[j], Point(xx - 1.0, yy));
				derivativeX /= 2.0;

				double t1 = get_subpixel
					(frame_pre[j], height[j], width[j], Point(xx, yy + 1.0));
				double t2 = get_subpixel(frame_pre[j], height[j],
					width[j], Point(xx, yy - 1.0));
			...
			while (iteration<max_iter&&opticalflow_residual>0.00001)
			{
				...
				for (double yy = Yleft; yy < Yright + 0.001; yy += 1.0)
				{
					...
					double pixelDifference = (get_subpixel(frame_pre[j],
						height[j], width[j], Point(xx, yy))
						- get_subpixel(frame_cur[j], height[j],
						width[j], Point(nextX, nextY)));
